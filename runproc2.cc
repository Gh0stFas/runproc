...
//pthread_mutex_t stdoe_mutex;
FILE *stdoe=NULL;
pthread_mutex_t logf_mutex;
FILE *logf=NULL;
pthread_mutex_t nbytes_mutex;
unsigned long nbytes_written=0;
pthread_mutex_t TtG_mutex;
int TIME_TO_GO=0;
...
void *rw_thread(void *arg){
  ...
  //pthread_mutex_lock(&stdoe_mutex);
  while(!feof(stdoe) && !ERROR_OCCURRED){
    ///////////////// Read ///////////////////////////
    ...
    // Let go of the logf mutex
    pthread_mutex_unlock(&logf_mutex);

    // Update the number of bytes written so the main thread can
    // make a log roll-over decision
    pthread_mutex_lock(&nbytes_mutex);
    nbytes_written += (unsigned long) nwritten;
    pthread_mutex_unlock(&nbytes_mutex);
    //////////////// Write ////////////////////
  }

  // Release the lock on stdoe
  //pthread_mutex_unlock(&stdoe_mutex);

  // Signal that we're done
  pthread_mutex_lock(&thread_finished_mutex);
  finished=1;
  pthread_mutex_unlock(&thread_finished_mutex);
...
int run(char *cmd, char *log_dir, char *log_prefix, char *log_arch_dir,  int instance, unsigned long rollover_size, int no_archive){
  ...
  logf = fopen(log_fname,"a");

  if(logf != NULL){
    // Write a log start marker
    fwrite(log_marker,sizeof(char),strlen(log_marker),logf);

    stdoe = popen2(cmd_r,"r",&child_pid);
    if(stdoe != NULL){
      printf("Child PID= %d\n",child_pid);
      pthread_t read_write_thread;
      pthread_create(&read_write_thread,NULL,rw_thread,(void *)NULL);
      ...
      // Join the thread
      void *status;
      pthread_join(read_write_thread,&status);
    }
    else perror("popen: ");
